...
exports.createProject = async (project) => {
    try {
        const contract = await getContractOrg2(USER); // user ต้องสมัครสมาชิคแล้วเท่านั้น
...
exports.donate = async (donation) => {
    try {
        const contract = await getContractOrg1(USER); // ถ้ายังไม่สมัครจะใช้ id ตั้งต้นหรือ ใช้ anonymous ของ firebase
